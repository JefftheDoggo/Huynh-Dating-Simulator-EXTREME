// -----------------------------------------------------------------------------
//
// [REDACTED] Dating Sim V0.0.0
// This code is coppied from the Taj Game so a lot of it is redundant
//
// -----------------------------------------------------------------------------
...
async function LBozo(player) {
    await cutscene(`LMAO U DIED!! TRASH!!! Now you have ${randint(10,99)} viruses on your computer. Lmao L Bozo. Imagine dying, can't relate. What a ${name} moment.`); // TODO: Add more insults
    ...
    await cutscene(`Have another forever loop!`);
    for (let i=0; i<=20; i++) {
        await cutscene(`Bet you wished you could skip cutscenes. Now you have to wait for this very long message to load. Surely you have better things to do with your life. THere is actually nothing more to see but if you don't believe me, you can keep clicking.`);
    }
...
async function scene1(player) {
    await cutscene(`It is lunchtime at school and you see [REDACTED] sitting alone at a table outside the [old building opposite the oval] building.`);
    let possibleActions = [
        'introduce yourself', 
        'move closer',
        'hide in the bushes',
    ...
    switch (await choice(`What do you do, ${player.name}?`, possibleActions, false, false, false)) {
        case 0:
        ...
        case 1:
            if (Math.random() > 0.7) {
                await cutscene(`You awkwardly attempt to shuffle closer. The key word is 'attempt'. In reality, you trip over your feet and faceplant into the unforgiving concrete.`);
                await cutscene(`Fortunately for you, [REDACTED] does not notice.`);
            } else {
                await cutscene(`You discretely approach [REDACTED]. You manage position yourself directy behind [REDACTED], giving you a good view of her backside.`);
                await cutscene(`You gained: +1 stealth.`);
                player.stealth += 1;
                possibleActions = [
                    'introduce yourself',
                    'tap her on the back', 
                ...
                switch (await choice(`What do you do now, ${player.name}?`, possibleActions, false, false, false)) {
                            ...
                            case 2:
                                ...
                                switch (await choice(`What do you say ${player.name}`, possibleActions, false, false, false)) {
                                    ...
                                    case 1:
                                        ...
                                        break;
                                    case 2:
                                        await cutscene(`${player.name}: Amogus!!!!)`);
                                        await cutscene(`[REDACTED] is very confused`);
                                        await cutscene(`Unfortunately for you, [REDACTED] does not like Amoung Us.`);
                                        await cutscene(`<p id="strong">YOU DIED: Amogus</p>`);
                                        await LBozo(player);
                                        await cutscene(`You gained: -1 progress with [REDACTED].`);
                                        await cutscene(`You gained: nickname 'Imposter'.`);
                                        player.progress -= 15;
                                    ...
                                    case 3:
                                        ...
                                        await cutscene(`<p id="strong">YOU DIED: Rejected</p>`);
                                        await LBozo(player);
                    ...
                    case 1:
                        ...
                        switch (await choice(`You tap [REDACTED] on the back before swiftly retreating to the _____?`, possibleActions, false, false, false)) {
                            case 0:
                                ...
                                if (player.reputation > 10) {
                                    ...
                                    possibleActions = [
                                        ...
                                        'I deeply apologies for disturing your lunch. Please forgive this humble man for his inappropriate actions.',
                                    ];
                                    switch (await choice(`[REDACTED]: What was that for?<br> ${player.name}: _____`, possibleActions, false, false, false)) {
                                        case 0:
                                        case 1:
                                            await cutscene(`[REDACTED]: I know it was you.`);
                                            await cutscene(`<p id="strong">YOU DIED: Social Anxiety</p>`);
                                            break;
                                        case 2:
                                            await cutscene(`[REDACTED]: Thats cap! I know it was you.`);
                                            await cutscene(`<p id="strong">YOU DIED: Social Anxiety</p>`);
                                            break;
                                        case 3:
                                            await cutscene(`[REDACTED]: Apology accepted.`);
                                            await cutscene(`[REDACTED]: Now don't bother me again.`);
                                            await cutscene(`<p id="strong">YOU DIED: No chance with [REDACTED]</p>`);
                                            player.progress = Math.min(0,player.progress);
                                            player.girlStatus = 'annoyed';
                                            return player;
                                        default:
                                            break;
                                    }
                                ...
                                return player
                            case 1:
                                await cutscene(`You retreat into the bushes.`);
                                await cutscene(`You crawl through the bushes with the gracefullness and subtlety of a rampaging bull. Thrashing wildly in the branches, you create enough noise making everybody stare at you, cementing your reputation as a creep in the bushes.`);
                                await cutscene(`You gained: -10 reputation.`);
                                await cutscene(`You gained: nickname 'Creep in the Bushes'.`);
                                player.reputation -= 10;
                                player.nickname = 'Creep in the Bushes';
                                await cutscene(`After making a fool of yourself, you decide to rethink your plans for winning [REDACTED]'s heart.`);
                                return player;
                            case 2:
                                await cutscene(`You retreat into the toilets, acting like a perfectly normal guy going to the toilets.`);
                                await cutscene(`In your haste to enter the toilets, you failed to notice that it was a female toilet.`);
                                // TODO: Unlock a new girl here
                                await cutscene(`<p id="strong">YOU DIED: Arrested for sexual harrassment</p>`);
                                await LBozo(player);
                                await cutscene(`You gained: -25 reputation.`);
                                await cutscene(`You gained: nickname 'Perverted spirit of the toilet'.`);
                                player.reputation -= 25;
                                player.nickname = 'Perverted spirit of the toilet';
                                await cutscene(`After making a fool of yourself, you decide to rethink your plans for winning [REDACTED]'s heart.`);
                                return player;
                            case 3:
                                await cutscene(`You run through the handball courts, angering the handball players.`);
                                // TODO: unlock a new girl here
                                await cutscene(`In your haste to get away, you fail to notice the fastball bowled by Taj the Terrorist heading right for your head.`);
                                await cutscene(`In a most unfortunate turn of events, momentum is indeed conserved in inelastic collisions and the ball transfers all of its momentum into your head.`);
                                await cutscene(`your face is once again introduced violently to the ground.`);
                                await cutscene(`<p id="strong">YOU DIED: Fall damage</p>`);
                                await LBozo(player);
                                await cutscene(`You gained: -5 reputation.`);
                                await cutscene(`You gained: nickname '[insert creative nickname here]'.`);
                                player.reputation -= 5;
                                player.nickname = '[insert creative nickname here]';
                                break;
                            default:
                                break;
                        }
                    ...
                    case 2:
                        ...
                        possibleActions = [
                            ...
                            'AMOGUS!!!', 
                            'Do you want some meth?',
                            'Do you want to know my Spec average?',
                        ];
                        switch (await choice(`How do you say, ${player.name}?`, possibleActions, false, false, false)) {
                            case 0:
                                await cutscene(`${player.name}: Hey girl, how are you today?`);
                                await cutscene(`Before you can do anything else, [a certain teacher who likes frogs] appears behind you and tells you off for sexual harrassment.`);
                                await cutscene(`You gained: +1 infringement.`);
                                player.infringments += 1;
                                await cutscene(`<p id="strong">YOU DIED: Death by [a certain teacher who likes frogs]</p>`);
                                await LBozo(player);
                                break;
                            case 1:
                                await cutscene(`You screan 'AMOUNGUS' so loudly that half the school heard you. [REDACTED] is covering her ears in pain and wincing.`);
                                await cutscene(`Unfortunately, a horde of year 7s arrive, all of them screaming 'amoungus'`);
                                await cutscene(`<p id="strong">YOU DIED: Cringe</p>`);
                                await LBozo(player);
                                break;
                            case 2:
                                await cutscene(`${player.name}: do you want some meth?<br>You use your incredible chemistry skills to synthesize a mole of C10H15N and offer it to [REDACTED].`);
                                await cutscene(`Unfortunately (or fortunately), [REDACTED] is a good citizen and does not like meth. [REDACTED] calls the police.`);
                                await cutscene(`<p id="strong">YOU DIED: Arrested for methemphetamine posession</p>`);
                                await LBozo(player);
                                break;
                            case 3:
                                await cutscene(`${player.name}: do you want to know my spec average?<br>[REDACTED]: um... sure.<br>${player.name}: I got 95% in spec! I'm godly at maths.<br>[REDACTED]: you got 74% in the first test right?`);
                                await cutscene(`<p id="strong">YOU DIED: EXPOSED!</p>`);
                                await LBozo(player);
                                break;
                            default:
                                break;
...
async function init() {
    ...
    hideText("console");
    hideText("controlPannel");
    showTitle(`<h1>Huynh-Dating-Simulator-EXTREME</h1>`);
